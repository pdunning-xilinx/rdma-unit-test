...
TEST_F(LoopbackUdQpTest, SendTrafficClass) {
  ...
  if (!verbs_util::peer_mode() || verbs_util::is_server()) {
    ...
    // On RoCE 2.0, the GRH (global routing header) is replaced by the IP
    // header.
    if (ip_family == AF_INET) {
      // The ipv4 header is located at the lower bytes bits of the GRH. The
      // higher 20 bytes are undefined. According to IPV4 header format and
      // ROCEV2 Annex A17.4.5.2 Last 2 bits might be used for ECN
...
TEST_F(LoopbackUdQpTest, SendRecvBatchedWr) {
  ...
  uint32_t rsge_size =
      std::min(active_mtu, (uint32_t)local.buffer.size() / batch_size);
  uint32_t lsge_size =
      rsge_size - sizeof(ibv_grh);  // Additional 40B for rsge is for the GRH
  ...
  if (!verbs_util::peer_mode() || verbs_util::is_server()) {
    ...
    for (uint32_t i = 0; i < batch_size; ++i) {
      rsge[i] = verbs_util::CreateSge(
          remote.buffer.subspan(
              /*offset=*/i * rsge_size, /*size=*/rsge_size),
          remote.mr);
  ...
  if (!verbs_util::peer_mode() || verbs_util::is_client()) {
    ...
    ASSERT_THAT(ah, NotNull());
    for (uint32_t i = 0; i < batch_size; ++i) {
      lsge[i] = verbs_util::CreateSge(
          local.buffer.subspan(
              /*offset=*/i * lsge_size, /*size=*/lsge_size),
          local.mr);
...
TEST_F(LoopbackUdQpTest, PollMultipleCqe) {
  ...
  if (!verbs_util::peer_mode() || verbs_util::is_client()) {
    ibv_sge send_sge = verbs_util::CreateSge(
        local.buffer.subspan(0, kPayloadLength), local.mr);
