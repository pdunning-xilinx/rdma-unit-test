...
absl::StatusOr<std::pair<ibv_wc_status, ibv_wc_status>> TwoSgeSendRecv(
  ...
  for (i=0; i<2; i++) {
    ...
    //In order to maintain unique values of the work request id across 4 requests,
    //using expressions i*2 and (i*2)+1, below.
    ibv_recv_wr recv = CreateRecvWr(/*wr_id=*/i*2, &dst_sge[i], /*num_sge=*/1);
    PostRecv(dst_qp, recv);
    ibv_send_wr send = CreateSendWr(/*wr_id=*/(i*2)+1, &src_sge, /*num_sge=*/1);
    PostSend(src_qp, send);
    ...
    if (src_completion.status != IBV_WC_SUCCESS || dst_completion.status != IBV_WC_SUCCESS) {
      goto label;
    }
    else {
      EXPECT_EQ(IBV_WC_SEND, src_completion.opcode);
