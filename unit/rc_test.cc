...
TEST_F(LoopbackRcQpTest, BadReadAddrRemote) {
  ...
  ibv_sge sge = verbs_util::CreateSge(local.buffer.span(), local.mr);
  ibv_send_wr read = verbs_util::CreateReadWr(
      /*wr_id=*/1, &sge, /*num_sge=*/1, remote.buffer.data()-32, remote.mr->rkey);
  verbs_util::PostSend(local.qp, read);
...
TEST_F(LoopbackRcQpTest, WriteBatchedWr) {
  ...
  for (uint32_t i = 0; i < batch_size; ++i) {
    sge[i] = verbs_util::CreateSge(local.buffer.subspan(
        /*offset=*/i*sge_size,/*size=*/sge_size), local.mr);
    write_batch[i] = verbs_util::CreateWriteWr(
        /*wr_id=*/i, &sge[i], /*num_sge=*/1, remote.buffer.data() + i*sge_size,
         remote.mr->rkey);
  ...
  if (!verbs_util::peer_mode() || verbs_util::is_server()) {
    for (uint32_t i = 0; i < batch_size; ++i) {
      rsge[i] = verbs_util::CreateSge(remote.buffer.subspan(
          /*offset=*/i*sge_size, /*size=*/sge_size), remote.mr);
